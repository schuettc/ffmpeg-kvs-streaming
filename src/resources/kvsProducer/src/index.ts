...
const KVS_STREAM_ARN = process.env.KVS_STREAM_ARN || '';
...
fastify.post('/processObject', async (request, reply) => {
  try {
    ...
    const { streamArn: streamArn, startFragmentNumber: startFragmentNumber } =
      await sendStream(bucketName, keyName);
    await reply.status(200).send({ streamArn, startFragmentNumber });
  } catch (error) {
...
async function sendStream(
  ...
  const { videoStream } = await processFfmpeg(filename);

  const {
    streamArn: videoStreamArn,
    startFragmentNumber: videoStreamStartFragmentNumber,
  } = await putMedia({
    stream: videoStream,
  });

  console.log('Video Stream ARN:', videoStreamArn);
  console.log(
    'Video Stream Start Fragment Number:',
    videoStreamStartFragmentNumber,
  );
  return {
    streamArn: videoStreamArn,
    startFragmentNumber: videoStreamStartFragmentNumber,
...
async function putMedia({ stream }: { stream: PassThrough }): Promise<{
  streamArn: string;
  startFragmentNumber: string;
}> {
  const streamArn = KVS_STREAM_ARN;
  if (!streamArn) {
  ...
  console.log('preparing to PutMedia');
  const axiosParams = {
    method: 'POST',
    timeout: 40 * 1000,
    url: reqUrl,
...
async function getStartFragmentNumber(
